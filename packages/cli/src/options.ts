...
import type { FlashcardDevMarkdown } from '@flashcard-dev/types'
...
const debug = _debug('flashcard-dev:options')
...
export interface SlidevServerOptions {
  onDataReload?: (newData: FlashcardDevMarkdown, data: FlashcardDevMarkdown) => void
}

export function getClientRoot() {
  return dirname(resolveImportPath('@flashcard-dev/client/package.json', true))
}
...
export async function resolveOptions(
  ...
  const addons = [] as string[]; // await getAddons(userRoot, data.config)
