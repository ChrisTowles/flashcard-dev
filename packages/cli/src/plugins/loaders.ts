  ...
  return [
    {
      ...
      configureServer(_server) {
        ...
        server.middlewares.use(async (req, res, next) => {
          ...
          if (type === 'json' && req.method === 'GET') {
            res.write(JSON.stringify(prepareSlideInfo(data.cards[idx])))
            return res.end()
          }
          if (type === 'json' && req.method === 'POST') {
            const body = await getBodyJson(req)
            const slide = data.cards[idx]
            hmrPages.add(idx)
      ...
      async handleHotUpdate(ctx) {
        ...
        if (data.cards.length !== newData.cards.length) {
          moduleIds.add('/@slidev/routes')
          range(newData.cards.length).map(i => hmrPages.add(i))
        }

        if (!equal(data.headmatter.defaults, newData.headmatter.defaults)) {
          moduleIds.add('/@slidev/routes')
          range(data.cards.length).map(i => hmrPages.add(i))
        }
        ...
        const length = Math.max(data.cards.length, newData.cards.length)

        for (let i = 0; i < length; i++) {
          const a = data.cards[i]
          const b = newData.cards[i]
          ...
          ctx.server.ws.send({
            ...
            data: {
              id: i,
              data: prepareSlideInfo(newData.cards[i]),
            },
          })
          hmrPages.add(i)
        }
        ...
        const vueModules = (
          await Promise.all(
            Array.from(hmrPages).map(async (i) => {
              ...
              try {
                const md = await transformMarkdown((await (<any>MarkdownPlugin.transform)(newData.cards[i]?.content, file)).code, i, newData)
      ...
      load(id): LoadResult | Promise<LoadResult> {
        ...
        if (id.startsWith(slidePrefix)) {
          ...
          if (match) {
            ...
            if (type === 'md') {
              return {
                code: data.cards[pageNo]?.content,
                map: { mappings: '' },
              }
            }
          }
